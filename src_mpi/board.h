...
public:
  ...
  enum
  {
    AllFields = 121, /* visible + ring of unvisible around */
  ...
  void playMove_stillOurTurn(const Move &m, int msecs = 0); // like playMove but keeps same player playing
  void adjustMoveNoAndTime();
  bool takeBack();              /* if not remembered, do nothing */
  bool takeBack_stillOurTurn(); // like takeBack but keeps same player playing
...
public:
  ...
  {
    std::memcpy(buffer, this, sizeof(Board));
  }

  void unpack(char *buffer)
  {
    std::memcpy(this, buffer, sizeof(Board));
  }
  int _BOARD_bestEval;
};
