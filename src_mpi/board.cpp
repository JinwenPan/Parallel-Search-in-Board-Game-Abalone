...
void Board::playMove_stillOurTurn(const Move &m, int msecs)
{
  int f, dir, dir2;
  int opponent = (color == color1) ? color2 : color1;
...
// void Board::adjustMoveNoAndTime(){
//   int opponent = (color == color1) ? color2:color1;
...
bool Board::takeBack_stillOurTurn()
{
  int f, dir, dir2;
  int opponent = color;
  Move &m = storedMove[storedLast];
...
int Board::validState()
{
  ...
  if (spyLevel > 2)
  {
    ...
    printf("Valid: %s (Color1 %d, Color2 %d, moveCount of %d: %d)\n",
           (res == empty) ? "empty" : (res == valid) ? "valid"
                                                     : "invalid",
...
const char *Board::stateDescription(int s)
{
  ...
  case valid1:
    return "O about to move...";
  case valid2:
    return "X about to move...";
  case timeout1:
    return "O timed out. X wins!";
  case timeout2:
    return "X timed out. O wins!";
  case win1:
...
Move &Board::bestMove()
{
  ...
  _BOARD_bestEval = (*_ss)._SS_bestEval;
  return best;
}

// Move &Board::getkthMove(int k)
// {
//   static Move kth_move;

//   kth_move = _ss->getkthMove(this, k);
//   return kth_move;
// }
...
/* Returns a human readable board representation.
...
bool Board::setState(char *s)
{
  ...
  while (*s)
  {
    if (*s == '#')
    {
      ...
      int col = -1;
      s++;
      // parse "#<moveNo> <color> - [(O|X): <count> (<secsToPlay>)]*"
      while ((*s >= '0') && (*s <= '9'))
        m = 10 * m + (*s++ - '0');
      newMoveNo = m;
      ...
      while (*s)
      {
        ...
        // saw a color before '-'? Use as current color
        if ((*s == '-') && (col > -1))
        {
          newColor = col;
          col = -1;
        ...
        if ((*s == '(') && (col > -1))
        {
          ...
          _msecsToPlay[col] = 1000 * secs + msecs;
          col = -1;
        }
        s++;
      }
    }
    if (row * 11 + 12 + column > AllFields)
      break;
    if (*s == '.' || *s == 'X' || *s == 'O' ||
        *s == 'x' || *s == 'o')
    {
      ...
      field[row * 11 + 12 + column] =
          (*s == 'O') ? color1 : (*s == 'o') ? color1
                             : (*s == 'X')   ? color2
                             : (*s == 'x')   ? color2
                                             : free;
      if (field[row * 11 + 12 + column] == color1)
        color1Count++;
      if (field[row * 11 + 12 + column] == color2)
        color2Count++;
      column++;
    }
    if (found && (*s == '\n'))
  ...
  // set current color to play
  if (newColor >= 0)
    color = newColor;
  else
    color = ((_moveNo % 2) == 0) ? color1 : color2; // assume O started game
