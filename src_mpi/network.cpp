...
int NetworkLoop::run()
{
    ...
    while(!exit_loop) {
        ...
        fd_set rfds = readfds;
        int ret = select(max_readfd+1, &rfds, NULL, NULL, ptv);
        if (ret < 0)
            if (verbose>1)
                perror("Error in select in NetworkLoop::run");

        if (ret > 0) {
            if (verbose>1) printf("NetworkLoop::run: Got an event...\n");
...
bool Connection::sendString(const char* str, int len)
{
    ...
    if (s < 0) {
        perror("Error in socket in Connection::sendString");
        return false;
    }
    ...
    if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR,
        ...
        perror("Error in setsockopt(SO_REUSEADDR) in Connection::sendString");
        return false;
    }
    ...
    while(::connect (s, (struct sockaddr *)&sin, sizeof (sin)) <0) {
        if (verbose)
            perror("Error in connect in Connection::sendString");
        if ((errno == EINTR) ||
            (errno == EADDRNOTAVAIL) ||
            (errno == EADDRINUSE)) {
            ...
            if (counter < 10000) {
                if (verbose)
                    printf("Retry ... waiting for %d ms\n", counter);

                struct timespec tim;
                tim.tv_sec = counter / 1000;
                tim.tv_nsec = 1000000 * (counter % 1000);
                nanosleep(&tim , 0);
                continue;
            }
        }
    ...
    while(len > 0) {
        ...
        if (written < 0) {
            perror("Error in write in Connection::sendString");
		    break;
        }
        str += written;
        len -= written;
...
void NetworkDomain::addConnection(const char* host, int port)
{
	...
	if ((strcmp(c->host, host) == 0) &&
	    ...
	    if (verbose)
		printf("NetworkDomain::addConnection: %s already existing\n",
		       c->addr());
	    c->reachable = true;
	    return;
	}
