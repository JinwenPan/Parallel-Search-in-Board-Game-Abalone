...
void ABIDStrategy::searchBestMove()
{
	...
	do
	{
		...
		while (1)
		{
			...
			currentValue = alphabeta(1, alpha, beta);
...
int ABIDStrategy::alphabeta(int depth, int alpha, int beta)
{
	...
	int maxType = (depth < _currentMaxDepth - 1) ? Move::maxMoveType : (depth < _currentMaxDepth) ? Move::maxPushType
																								  : Move::maxOutType;
	...
	while (1)
	{
		...
		// if (value < currentValue)
		{
			...
			// if (currentValue < -14900 || currentValue <= alpha)
			{
				if (_sc)
